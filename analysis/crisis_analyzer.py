...
class CrisisAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            "Performance optimizations integrated - targeting 500ms analysis time"
        )
        ...
        logger.info(
            f"   Helper files: 5 loaded (ensemble, scoring, pattern, context, tracking)"
        )
        logger.info(
            f"   Zero-shot models: Implemented with ZeroShotManager integration"
        )
        logger.info(
            f"   Analysis flow tracking: {'Enabled' if self.tracking_helper.enable_tracking else 'Disabled'}"
        )
        logger.info(
            f"   SharedUtilitiesManager: {'Available' if shared_utilities_manager else 'Not available'}"
        )
        logger.info(
            f"   LearningSystemManager: {'Available' if learning_system_manager else 'Not available'}"
        )
        logger.info(
            f"   ZeroShotManager: {'Available' if zero_shot_manager else 'Not available'}"
        )
    ...
    async def analyze_crisis(
        self, message: str, user_id: str, channel_id: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Initialize tracking
        tracking = self.tracking_helper.init_analysis_tracking(
        ...
        if not self.tracking_helper.enable_tracking:
            logger.debug(
                "Tracking disabled - using ZeroShot analysis without detailed tracking"
            )
            try:
                ...
                optimized_result = (
                    await self.tracking_helper.execute_zero_shot_analysis(
                        message, user_id, channel_id
                    )
                )
                ...
                optimized_result["crisis_level"] = self.apply_crisis_thresholds(
                    crisis_score, self.performance_optimizer._cached_ensemble_mode
                )
                ...
                if self.learning_system_manager:
                    try:
                        learning_result = (
                            self.learning_system_manager.apply_learning_adjustments(
                                optimized_result, user_id, channel_id
                            )
                        )
                        optimized_result.update(
                            {
                                "learning_adjusted_score": learning_result.get(
                                    "adjusted_score",
                                    optimized_result.get("crisis_score", 0.0),
                                ),
                                "learning_metadata": learning_result.get(
                                    "metadata", {}
                                ),
                ...
                return self.tracking_helper.finalize_tracking(
                    tracking, optimized_result
                )
        ...
        logger.info(
            f"Phase 4a Step 2: Starting comprehensive crisis analysis for user {user_id} in channel {channel_id}"
        )
        ...
        try:
            ...
            if ensemble_enabled:
                ...
                self.tracking_helper.update_tracking_step(
                    tracking, "step_1_zero_shot_ai", "started"
                )
                try:
                    ensemble_result = (
                        await self.tracking_helper.execute_zero_shot_analysis(
                    ...
                    self.tracking_helper.update_tracking_step(
                        ...
                        "completed",
                        {
                            ...
                            "individual_scores": ensemble_result.get(
                                "individual_results", {}
                            ),
                            ...
                            "zero_shot_labels_info": ensemble_result.get(
                                "zero_shot_labels_info", {}
                            ),
                        },
                    )
                    ...
                    self.tracking_helper.update_tracking_step(
                        tracking, "step_1_zero_shot_ai", "failed", error=e
                    )
                ...
                self.tracking_helper.update_tracking_step(
                    tracking, "step_2_pattern_enhancement", "started"
                )
                try:
                    pattern_result = (
                        await self.tracking_helper.execute_pattern_enhancement(
                            message, ensemble_result
                        )
                    )
                    self.tracking_helper.update_tracking_step(
                        tracking,
                        ...
                        "completed",
                        {
                            "patterns_matched": pattern_result.get(
                                "patterns_found", []
                            ),
                            "pattern_categories": pattern_result.get(
                                "detected_categories", []
                            ),
                            "enhancement_applied": pattern_result.get(
                                "enhancement_applied", False
                            ),
                            "confidence_boost": pattern_result.get(
                    ...
                    self.tracking_helper.update_tracking_step(
                        tracking, "step_2_pattern_enhancement", "failed", error=e
                    )
                    pattern_result = {"enhancement_applied": False}

                # STEP 3: Learning System Adjustments (ADAPTIVE LEARNING)
                self.tracking_helper.update_tracking_step(
                    tracking, "step_3_learning_adjustments", "started"
                )
                try:
                    if self.learning_system_manager:
                        learning_result = (
                            await self.tracking_helper.execute_learning_adjustments(
                                ensemble_result, pattern_result, user_id, channel_id
                            )
                        )
                        self.tracking_helper.update_tracking_step(
                            tracking,
                            ...
                            "completed",
                            {
                                "threshold_adjustments": learning_result.get(
                                    "adjustments", {}
                                ),
                                "confidence_modifications": learning_result.get(
                        ...
                        logger.debug(
                            "Step 3: Learning adjustments applied successfully"
                        )
                    else:
                        self.tracking_helper.update_tracking_step(
                            tracking,
                    ...
                    self.tracking_helper.update_tracking_step(
                        tracking, "step_3_learning_adjustments", "failed", error=e
                    )
                    learning_result = {"learning_applied": False}

                # Combine results
                final_result = self.tracking_helper.combine_analysis_results(
                    ensemble_result,
                    pattern_result,
                    learning_result,
                    self.performance_optimizer._cached_ensemble_mode,
                )
                ...
                try:
                    final_result = await self.pattern_helper.basic_crisis_analysis(
                        message, user_id, channel_id, time.time()
                    )
                    self.tracking_helper.update_tracking_step(
                        tracking,
                        "step_2_pattern_enhancement",
            ...
            final_result = self.tracking_helper.finalize_tracking(
                tracking, final_result
            )

            total_time = final_result.get("tracking_summary", {}).get(
                "total_processing_time_ms", 0
            )
            logger.info(
                f"Phase 4a Step 2: Crisis analysis completed in {total_time:.1f}ms"
            )
    ...
    async def analyze_message(
        ...
        """
        logger.debug(
            "analyze_message called - delegating to enhanced analyze_crisis for backward compatibility"
        )
    ...
    # ========================================================================
    def get_analysis_crisis_thresholds(
        self, mode: str = "consensus"
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if self.crisis_threshold_manager:
                try:
                    thresholds = (
                        self.crisis_threshold_manager.get_ensemble_thresholds_for_mode(
                            mode
                        )
                    )
                    if thresholds and all(
                        ...
                        logger.debug(
                            f"Got thresholds from CrisisThresholdManager for mode '{mode}': {thresholds}"
                        )
                        return thresholds
                    else:
                        logger.warning(
                            f"CrisisThresholdManager returned invalid thresholds: {thresholds}"
                        )
                except Exception as e:
                ...
                try:
                    ...
                    if threshold_config:
                        ...
                        for key, default_val in defaults.items():
                            try:
                                safe_thresholds[key] = float(
                                    threshold_config.get(key, default_val)
                                )
                            except (ValueError, TypeError):
                                logger.warning(
                                    f"Invalid threshold value for {key}, using default {default_val}"
                                )
                                safe_thresholds[key] = default_val

                        logger.debug(
                            f"Got thresholds from UnifiedConfig for mode '{mode}': {safe_thresholds}"
                        )
        ...
        try:
            if self.shared_utilities_manager:
                return self.shared_utilities_manager.get_config_section_safely(
                    "analysis_config",
                    "timeouts",
                    {"model_analysis": 10, "pattern_analysis": 5, "total_analysis": 30},
                )
    ...
    def get_analysis_confidence_boosts(self) -> Dict[str, float]:
        ...
        try:
            if self.shared_utilities_manager:
                ...
                logger.warning(
                    "No config manager available - using default confidence boosts"
                )
                return {
                    "pattern_match": 0.1,
                    "context_boost": 0.15,
    ...
    def get_analysis_pattern_weights(self) -> Dict[str, float]:
        ...
        try:
            if self.unified_config_manager:
                ...
                logger.warning(
                    "No config manager available - using default pattern weights"
                )
                return {
                    "patterns_crisis": 0.6,
                    "community_patterns": 0.3,
    ...
    def get_analysis_algorithm_parameters(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                hasattr(self, "performance_optimizer")
                and self.performance_optimizer
                and hasattr(self.performance_optimizer, "_cached_model_weights")
                and hasattr(self.performance_optimizer, "_cached_model_weights")
            ):
                ...
                logger.debug(
                    f"🎯 Using cached weights from performance optimizer: {cached_weights}"
                )
            ...
            if self.shared_utilities_manager:
                ...
                logger.warning(
                    "No config manager available - using default algorithm parameters"
                )
                return {
                    "ensemble_weights": [0.4, 0.3, 0.3],
                    "score_normalization": "sigmoid",
    ...
    # ========================================================================
    def apply_crisis_thresholds(
        self, confidence: float, mode: str = "consensus"
    ) -> str:
        """
        ...
        try:
            ...
            if self.crisis_threshold_manager:
                try:
                    if hasattr(self.crisis_threshold_manager, "determine_crisis_level"):
                        return self.crisis_threshold_manager.determine_crisis_level(
                            confidence, mode
                        )
                    elif hasattr(self.crisis_threshold_manager, "apply_thresholds"):
                        return self.crisis_threshold_manager.apply_thresholds(
                            confidence, mode
                        )
                    elif hasattr(self.crisis_threshold_manager, "get_crisis_level"):
                        return self.crisis_threshold_manager.get_crisis_level(
                            confidence, mode
                        )
                    else:
                        logger.debug(
                            "CrisisThresholdManager has no known threshold application method - using consolidated logic"
                        )
                except Exception as e:
                    logger.warning(
                        f"CrisisThresholdManager threshold application failed: {e}"
                    )
            ...
            if self.learning_system_manager:
                try:
                    adjusted_confidence = (
                        self.learning_system_manager.apply_threshold_adjustments(
                            confidence, mode
                        )
                    )
                    logger.debug(
                        f"Learning adjustment: {confidence:.3f} → {adjusted_confidence:.3f}"
                    )
    ...
    def calculate_crisis_level_from_confidence(
        self, confidence: float, mode: str = "default"
    ) -> str:
        """
    ...
    def analyze_message_with_learning(
        self, message: str, user_id: str, channel_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            # Get base analysis
            base_result = self.perform_ensemble_crisis_analysis(
                message, user_id, channel_id
            )
            ...
            if self.learning_system_manager:
                ...
                logger.debug(
                    f"Learning system applied adjustments: {learning_result.get('adjustments', {})}"
                )
            else:
    ...
    def process_analysis_feedback(
        ...
        try:
            if self.learning_system_manager:
                ...
                logger.info(
                    f"Learning feedback processed: {feedback_type} for user {user_id}"
                )
            else:
    ...
    def perform_ensemble_crisis_analysis(
        self, message: str, user_id: str, channel_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            optimized_result = self.performance_optimizer.optimized_ensemble_analysis(
                message, user_id, channel_id
            )
    ...
    def _original_ensemble_analysis(
        self, message: str, user_id: str, channel_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            base_result = asyncio.run(
                self.ensemble_helper.perform_ensemble_analysis(
                    message, user_id, channel_id, start_time
                )
            )
    ...
    def _safe_analysis_execution(
        ...
        """
        try:
            if self.shared_utilities_manager and hasattr(
                self.shared_utilities_manager, "execute_safely"
            ):
                ...
                logger.debug(
                    f"Direct execution for {operation_name} - SharedUtilitiesManager execute_safely not available"
                )
    ...
    def _validate_analysis_input(
        ...
        try:
            if self.shared_utilities_manager:
                validations = [
                    self.shared_utilities_manager.validate_type(
                        message, str, "message"
                    ),
                    self.shared_utilities_manager.validate_type(
                        user_id, str, "user_id"
                ...
                return (
                    ...
                    and len(user_id.strip()) > 0
                    and isinstance(channel_id, str)
                    and len(channel_id.strip()) > 0
                )
        except Exception as e:
...
logger.info(
    "ENHANCED CrisisAnalyzer v3.1-4a-2-1 loaded - Comprehensive analysis flow tracking implemented"
)
