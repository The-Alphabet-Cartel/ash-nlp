...
********************************************************************************
User API Endpoints for Ash NLP Service
---
FILE VERSION: v5.0
...
def add_ensemble_endpoints(
    ...
    if crisis_threshold_manager:
        try:
            ...
            logger.info(
                f"CrisisThresholdManager integrated - Current mode: {current_mode}"
            )
        ...
        logger.warning(
            "CrisisThresholdManager not provided - using fallback thresholds"
        )
    ...
    async def analyze_message_clean(request: models["MessageRequest"]):
        ...
        try:
            ...
            logger.debug(
                f"Clean Architecture: Analyzing message from user {request.user_id}"
            )

            # Enhanced validation
            if not crisis_analyzer:
                raise HTTPException(
                    status_code=500, detail="Crisis Analyzer not available"
                )
            ...
            try:
                ...
                logger.debug(
                    f"Complete analysis via CrisisAnalyzer: {complete_analysis.get('method', 'unknown')}"
                )
            ...
            crisis_score = analysis_results.get(
                "crisis_score", 0.0
            )  # ✅ FIX: Extract crisis_score
            ...
            logger.debug(
                f"complete_analysis crisis_score: {complete_analysis.get('crisis_score', 'missing')}"
            )
            logger.debug(
                f"analysis_results crisis_score: {analysis_results.get('crisis_score', 'missing')}"
            )

            # Enhanced fallback to top-level keys for backward compatibility
            if not analysis_results:
                logger.debug(
                    "Falling back to top-level keys for backward compatibility"
                )
                crisis_level = complete_analysis.get("crisis_level", "none")
                crisis_score = complete_analysis.get(
                    "crisis_score", 0.0
                )  # ✅ FIX: Fallback for crisis_score
                confidence_score = complete_analysis.get("confidence_score", 0.0)

            # Validate extracted values
            if not isinstance(crisis_level, str) or crisis_level not in [
                "none",
                "low",
                "medium",
                "high",
            ...
            if (
                not isinstance(crisis_score, (int, float))
                or crisis_score < 0
                or crisis_score > 1
            ):
            ...
            if (
                ...
                logger.warning(
                    f"Invalid confidence_score '{confidence_score}', using 0.0"
                )
                confidence_score = 0.0

            logger.debug(
                f"Extracted crisis_level={crisis_level}, crisis_score={crisis_score:.3f}, confidence_score={confidence_score:.3f}"
            )
            logger.debug(
                f"Analysis structure: has_analysis_results={bool(analysis_results)}"
            )
            ...
            try:
                response = models[
                    ...
                    detected_categories=complete_analysis.get(
                        "detected_categories", []
                    ),
                    method=complete_analysis.get("method", "crisis_analyzer"),
                    processing_time_ms=processing_time_ms,
                    model_info=complete_analysis.get(
                        "model_info", "Clean Architecture - CrisisAnalyzer Complete"
                    ),
                    reasoning=complete_analysis.get(
                        "reasoning",
                        "Single analysis via CrisisAnalyzer with enhanced validation",
                    ),
                    analysis={
                        ...
                        "context_analysis_available": bool(
                            analysis_results.get("context_analysis")
                        ),
                    },
                )
            except Exception as e:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Response creation failed: {str(e)}"
                )
            ...
            logger.debug(
                f"Clean Architecture Result: {crisis_level} (crisis_score: {crisis_score:.3f}, conf: {confidence_score:.3f}) via {method}"
            )
            logger.debug(f"Feature flags applied: {feature_flags}")
            logger.info(
                f"API response extraction successful with validation - crisis_score field included"
            )
            ...
            processing_time_ms = (
                (time.time() - start_time) * 1000 if "start_time" in locals() else 0.0
            )
    ...
    async def ensemble_health_check():
        ...
        try:
            ...
            try:
                ...
                model_info = (
                    model_coordination_manager.get_model_info() if models_loaded else {}
                )
            except Exception as e:
            ...
            if pattern_detection_manager:
                try:
                    pattern_info = {
                        "patterns_loaded": len(
                            pattern_detection_manager.get_available_patterns()
                        ),
            ...
            overall_status = (
                ...
                if (
                    models_loaded
                    and pattern_manager_status
                    and threshold_manager_status
                )
                else "degraded"
            )
            ...
            return {
                ...
                "components": {
                    ...
                    "pattern_analysis": {
                        "status": "healthy"
                        if pattern_manager_status
                        else "not_available",
                        "details": pattern_info,
                    },
                    "crisis_threshold": {
                        "status": "healthy"
                        if threshold_manager_status
                        else "not_available",
    ...
    async def set_ensemble_weights(
        ...
        try:
            ...
            if hasattr(crisis_analyzer, "performance_optimizer"):
                ...
                old_mode = getattr(
                    crisis_analyzer.performance_optimizer,
                    "_cached_ensemble_mode",
                    "unknown",
                )
                ...
                crisis_analyzer.performance_optimizer._cached_ensemble_mode = (
                    ensemble_mode
                )
                ...
                logger.info(
                    f"   New weights: {crisis_analyzer.performance_optimizer._cached_model_weights}"
                )
                ...
                if (
                    hasattr(crisis_analyzer, "model_coordination_manager")
                    and crisis_analyzer.model_coordination_manager
                ):
                    try:
                        manager_mode = crisis_analyzer.model_coordination_manager.get_ensemble_mode()
                        logger.info(
                            f"🔍   ModelCoordinationManager mode: {manager_mode}"
                        )
                    except Exception as e:
            ...
            logger.info(
                f"   Cache Updated: {hasattr(crisis_analyzer, 'performance_optimizer')}"
            )
    ...
    async def refresh_ensemble_weights(force_reload: bool = False):
        ...
        try:
            ...
            env_vars = {
                "NLP_MODEL_DEPRESSION_WEIGHT": os.getenv(
                    "NLP_MODEL_DEPRESSION_WEIGHT", "not_set"
                ),
                "NLP_MODEL_SENTIMENT_WEIGHT": os.getenv(
                    "NLP_MODEL_SENTIMENT_WEIGHT", "not_set"
                ),
                "NLP_MODEL_DISTRESS_WEIGHT": os.getenv(
                    "NLP_MODEL_DISTRESS_WEIGHT", "not_set"
                ),
            ...
            if hasattr(crisis_analyzer, "performance_optimizer"):
                try:
                    if force_reload:
                        ...
                        if not success:
                            logger.warning(
                                "Normal refresh failed, trying nuclear reload..."
                            )
                            success = crisis_analyzer.performance_optimizer.force_environment_variable_reload()
                    ...
                    logger.info(
                        f"Performance optimizer cache refreshed via {method}: {success}"
                    )
            ...
            raise HTTPException(
                status_code=500, detail=f"Weight refresh failed: {str(e)}"
            )
    ...
    if crisis_threshold_manager:
        try:
            ...
            logger.info(
                f"Ensemble endpoints configured with {current_mode} mode thresholds"
            )
