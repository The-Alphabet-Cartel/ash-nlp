# Ash-NLP v5.0 Complete

**FILE VERSION**: v1.1  
**CREATED**: 2026-01-02  
**UPDATED**: 2026-01-02  
**STATUS**: ✅ PRODUCTION READY  
**Repository**: https://github.com/the-alphabet-cartel/ash-nlp  
**Community**: The Alphabet Cartel - https://discord.gg/alphabetcartel | https://alphabetcartel.org

---

## 🎉 Ash-NLP v5.0 - COMPLETE

**Completion Date**: 2026-01-02

Ash-NLP v5.0 represents a complete rewrite of the crisis detection backend, implementing a sophisticated multi-model ensemble architecture with weighted decision engine for accurate crisis classification in Discord community communications.

---

## Version Overview

| Attribute | Value |
|-----------|-------|
| Version | v5.0.7 |
| Phases Completed | 6 |
| Total Tests | 512 |
| Skipped Tests | 2 (model loading integration) |
| Architecture | Multi-Model Ensemble |
| Primary Model | BART Zero-Shot Classification |

---

## Phase Summary

| Phase | Focus | Status |
|-------|-------|--------|
| Phase 1 | Core Infrastructure | ✅ Complete |
| Phase 2 | Model Integration | ✅ Complete |
| Phase 3 | Configuration Management | ✅ Complete |
| Phase 4 | Ensemble Coordination | ✅ Complete |
| Phase 5 | Context Analysis | ✅ Complete |
| Phase 6 | Future Enhancements | ✅ Complete |

---

## Core Architecture

### Multi-Model Ensemble
```
Message → BART (0.50) ─────────────┐
        → Sentiment (0.25) ────────┼→ Weighted Decision Engine → Crisis Assessment
        → Irony (0.15) ────────────┤
        → Emotions (0.10) ─────────┘
```

### Model Weights
| Model | Weight | Purpose |
|-------|--------|---------|
| BART Zero-Shot | 0.50 | Primary semantic crisis detection |
| Cardiff Sentiment | 0.25 | Emotional tone analysis |
| Cardiff Irony | 0.15 | Sarcasm/irony detection |
| RoBERTa Emotions | 0.10 | Fine-grained emotion classification |

---

## Key Features

### Crisis Detection
- ✅ Multi-model ensemble with weighted consensus
- ✅ Five severity levels: critical, high, medium, low, safe
- ✅ Confidence scoring with explainability
- ✅ Conflict detection and resolution

### Context Analysis
- ✅ Message history trajectory analysis
- ✅ Escalation pattern detection
- ✅ Temporal risk factors (late night, weekends)
- ✅ User timezone support
- ✅ Per-severity escalation thresholds

### Alerting & Integration
- ✅ Discord webhook alerts with throttling
- ✅ Severity-based alert colors and emojis
- ✅ ASCII disagreement charts for conflicts
- ✅ Message length validation and truncation

### Performance & Reliability
- ✅ Model warm-up with detailed timing
- ✅ Retry logic with exponential backoff
- ✅ Timeout handling for inference
- ✅ Response caching
- ✅ Prometheus metrics (optional)

### Developer Experience
- ✅ Clean Architecture v5.1 compliant
- ✅ Factory function patterns
- ✅ Comprehensive test coverage (512 tests)
- ✅ Structured JSON logging
- ✅ Interactive API documentation

---

## API Endpoints

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/analyze` | POST | Analyze message for crisis indicators |
| `/health` | GET | Health check with model status |
| `/status` | GET | Detailed system status |
| `/docs` | GET | Interactive API documentation |

---

## Configuration

### Environment Variables
- `NLP_ENVIRONMENT`: production, testing, development
- `NLP_LOG_LEVEL`: DEBUG, INFO, WARNING, ERROR
- `NLP_LOG_FORMAT`: json, human

### Config Files
- `src/config/default.json` - Default configuration
- `src/config/production.json` - Production overrides
- `src/config/testing.json` - Testing overrides
- `src/config/context_config.json` - Context analysis settings

---

## Deployment

### Docker
```bash
docker compose build --no-cache
docker compose up -d
```

### Port
- Default: 30880

### Health Check
```bash
curl http://localhost:30880/health
```

---

## Test Coverage

| Suite | Tests | Status |
|-------|-------|--------|
| Phase 3 | 92 | ✅ Pass |
| Phase 4 | 149 | ✅ Pass |
| Phase 5 | 144 | ✅ Pass |
| Phase 6 | 79 | ✅ Pass (2 skipped) |
| Integration | 47 | ✅ Pass |
| **Total** | **512** | **✅ All Pass** |

**512 tests passed, 2 skipped (model loading integration tests)**

---

## Future Roadmap (v5.1)

- **FE-006**: Historical Pattern Learning
  - Store anonymized escalation sequences
  - Train lightweight pattern recognition
  - Improve prediction accuracy over time

---

## Documentation

| Document | Location |
|----------|----------|
| Clean Architecture Charter | `docs/clean_architecture_charter.md` |
| Changelog | `CHANGELOG.md` |
| Phase 6 Complete | `docs/v5.0/Phase6/phase6_complete.md` |
| Phase 6 Enhancements | `docs/v5.0/Phase6/phase6_enhancements.md` |
| API Response Reference | `docs/api_response_reference.md` |
| API Documentation | `http://localhost:30880/docs` |

---

## Acknowledgments

Ash-NLP v5.0 was built to protect and support The Alphabet Cartel community - an LGBTQIA+ Discord community centered around gaming, political discourse, activism, and societal advocacy.

**Crisis detection saves lives.**

---

**Built with care for chosen family** 🏳️‍🌈
