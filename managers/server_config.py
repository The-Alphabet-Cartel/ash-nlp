...
class ServerConfigManager:
    ...
    def __init__(self, unified_config_manager):
        ...
        logger.info(
            "ServerConfigManager v3.1e-5.5 initialized - Phase 3e patterns applied"
        )
    ...
    def _load_server_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "Server configuration loaded from JSON with environment overrides"
            )
    ...
    def _get_setting_with_defaults(
        self, section: str, subsection: str, setting: str, default: Any
    ) -> Any:
        """
        ...
        """
        try:
            config_section = self.server_config.get("server_configuration", {}).get(
                section, {}
            )
            ...
            logger.warning(
                f"Error getting setting {section}.{subsection}.{setting}: {e}"
            )
            return default
    ...
    def get_network_settings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            settings = {
                "host": self._get_setting_with_defaults(
                    "network_settings", None, "host", "0.0.0.0"
                ),
                "port": self._get_setting_with_defaults(
                    "network_settings", None, "port", 8881
                ),
                "enable_ssl": self._get_setting_with_defaults(
                    "network_settings", None, "enable_ssl", False
                ),
                "ssl_cert_path": self._get_setting_with_defaults(
                    "network_settings", None, "ssl_cert_path", "./certs/server.crt"
                ),
                "ssl_key_path": self._get_setting_with_defaults(
                    "network_settings", None, "ssl_key_path", "./certs/server.key"
                ),
            }

            # Validate port range
            if (
                not isinstance(settings["port"], int)
                or settings["port"] < 1
                or settings["port"] > 65535
            ):
    ...
    def get_application_settings(self) -> Dict[str, Any]:
        ...
        try:
            settings = {
                "debug_mode": self._get_setting_with_defaults(
                    "application_settings", None, "debug_mode", False
                ),
                "workers": self._get_setting_with_defaults(
                    "application_settings", None, "workers", 1
                ),
                "reload_on_changes": self._get_setting_with_defaults(
                    "application_settings", None, "reload_on_changes", False
                ),
                "access_log": self._get_setting_with_defaults(
                    "application_settings", None, "access_log", True
                ),
                "error_log": self._get_setting_with_defaults(
                    "application_settings", None, "error_log", True
                ),
            }
            ...
            if not isinstance(settings["workers"], int) or settings["workers"] < 1:
                logger.warning(
                    f"Invalid worker count {settings['workers']}, using default 1"
                )
                settings["workers"] = 1
            elif settings["workers"] > 16:
                logger.warning(
                    f"High worker count {settings['workers']} may impact performance"
                )
    ...
    def get_performance_settings(self) -> Dict[str, Any]:
        ...
        try:
            settings = {
                "max_concurrent_requests": self._get_setting_with_defaults(
                    "performance_settings", None, "max_concurrent_requests", 20
                ),
                "request_timeout": self._get_setting_with_defaults(
                    "performance_settings", None, "request_timeout", 40
                ),
                "worker_timeout": self._get_setting_with_defaults(
                    "performance_settings", None, "worker_timeout", 60
                ),
                "keep_alive_timeout": self._get_setting_with_defaults(
                    "performance_settings", None, "keep_alive_timeout", 2
                ),
                "max_request_size": self._get_setting_with_defaults(
                    "performance_settings", None, "max_request_size", "10MB"
                ),
            }

            # Validate timeout values
            if (
                not isinstance(settings["request_timeout"], int)
                or settings["request_timeout"] < 1
            ):
                logger.warning(
                    f"Invalid request timeout {settings['request_timeout']}, using default 40"
                )
                settings["request_timeout"] = 40
            elif settings["request_timeout"] < 5:
                logger.warning(
                    f"Very low request timeout {settings['request_timeout']}s may cause premature timeouts"
                )

            if (
                not isinstance(settings["max_concurrent_requests"], int)
                or settings["max_concurrent_requests"] < 1
            ):
                logger.warning(
                    f"Invalid max concurrent requests {settings['max_concurrent_requests']}, using default 20"
                )
    ...
    def get_security_settings(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "rate_limiting": {
                    "enable_rate_limiting": self._get_setting_with_defaults(
                        "security_settings",
                        "rate_limiting",
                        "enable_rate_limiting",
                        True,
                    ),
                    "requests_per_minute": self._get_setting_with_defaults(
                        "security_settings", "rate_limiting", "requests_per_minute", 60
                    ),
                    "requests_per_hour": self._get_setting_with_defaults(
                        "security_settings", "rate_limiting", "requests_per_hour", 1000
                    ),
                    "burst_size": self._get_setting_with_defaults(
                        "security_settings", "rate_limiting", "burst_size", 100
                    ),
                },
                "cors": {
                    "enable_cors": self._get_setting_with_defaults(
                        "security_settings", "cors", "enable_cors", True
                    ),
                    "allowed_origins": self._get_setting_with_defaults(
                        "security_settings", "cors", "allowed_origins", ["*"]
                    ),
                    "allowed_methods": self._get_setting_with_defaults(
                        "security_settings",
                        "cors",
                        "allowed_methods",
                        ["GET", "POST", "OPTIONS"],
                    ),
                    "allowed_headers": self._get_setting_with_defaults(
                        "security_settings",
                        "cors",
                        "allowed_headers",
    ...
    def get_deployment_profiles(self) -> Dict[str, Any]:
        ...
        try:
            ...
            profiles = self.unified_config.get_config_section(
                "server_config", "deployment_profiles", {}
            )
            if not profiles:
    ...
    def get_monitoring_settings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            monitoring = self.unified_config.get_config_section(
                "server_config", "server_configuration.monitoring", {}
            )
            if not monitoring:
                # Fallback to direct server_config access
            ...
            return {
                "health_check_endpoint": monitoring.get(
    ...
    def get_operational_settings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "max_concurrent_requests": performance.get(
                    "max_concurrent_requests", 20
                ),
    ...
    def validate_server_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            network = self.get_network_settings()
            if (
                not isinstance(network["port"], int)
                or network["port"] < 1024
                or network["port"] > 65535
            ):
                validation_results["errors"].append(
                    f"Invalid port: {network['port']} (must be 1024-65535)"
                )
            ...
            application = self.get_application_settings()
            if (
                not isinstance(application["workers"], int)
                or application["workers"] < 1
            ):
                validation_results["errors"].append(
                    f"Invalid worker count: {application['workers']} (must be >= 1)"
                )
                validation_results["valid"] = False
            elif application["workers"] > 16:
                validation_results["warnings"].append(
                    f"High worker count: {application['workers']} (recommended: 1-8)"
                )

            # Validate performance settings with enhanced checks
            performance = self.get_performance_settings()
            if (
                not isinstance(performance["max_concurrent_requests"], int)
                or performance["max_concurrent_requests"] < 1
            ):
                validation_results["errors"].append(
                    f"Invalid max concurrent requests: {performance['max_concurrent_requests']}"
                )
                validation_results["valid"] = False
            elif performance["max_concurrent_requests"] > 1000:
                validation_results["warnings"].append(
                    f"Very high concurrent requests: {performance['max_concurrent_requests']} (may impact performance)"
                )

            if (
                not isinstance(performance["request_timeout"], int)
                or performance["request_timeout"] < 1
            ):
                validation_results["errors"].append(
                    f"Invalid request timeout: {performance['request_timeout']}"
                )
                validation_results["valid"] = False
            elif performance["request_timeout"] < 5:
                validation_results["warnings"].append(
                    f"Very low request timeout: {performance['request_timeout']}s (may cause premature timeouts)"
                )
            ...
            if (
                not isinstance(rate_limiting.get("requests_per_minute", 0), int)
                or rate_limiting.get("requests_per_minute", 0) < 1
            ):
                validation_results["warnings"].append(
                ...
                validation_results["warnings"].append(
                    "High rate limiting threshold may not provide adequate protection"
                )

            logger.info(
                f"Server configuration validation: {'PASSED' if validation_results['valid'] else 'FAILED'}"
            )
    ...
    def get_configuration_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "initialization_status": "error"}
...
def create_server_config_manager(unified_config_manager) -> ServerConfigManager:
    ...
    if not unified_config_manager:
        raise ValueError(
            "UnifiedConfigManager is required for ServerConfigManager factory"
        )
...
logger.info(
    "ServerConfigManager v3.1e-5.5-2 loaded - Phase 3e Sub-step 5.5 cleanup complete with enhanced patterns"
)
