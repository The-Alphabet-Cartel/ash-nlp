...
class SettingsManager:
    ...
    def __init__(
        ...
        logger.info(
            "✅ SettingsManager v3.1e-5.5 initialized - Phase 3e cleanup complete"
        )
    ...
    def _load_runtime_settings(self):
        ...
        try:
            ...
            phase_status = self.unified_config.get_config_section(
                ...
                {"unified_config_manager": "operational", "manager_cleanup": "active"},
            )

            self.runtime_settings = {"server": SERVER_CONFIG}
    ...
    def _validate_manager_integration(self):
        ...
        logger.info(
            f"✅ Available managers ({len(available_managers)}): {available_managers}"
        )
        if missing_managers:
            logger.warning(
                f"⚠️ Missing managers ({len(missing_managers)}): {missing_managers}"
            )

        # Validate that UnifiedConfigManager is available (critical for operation)
        if not self.unified_config:
            raise ValueError(
                "UnifiedConfigManager is required for SettingsManager operation"
            )
    ...
    def get_environment_bool(self, var_name: str, default: bool = False) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Error getting boolean environment variable {var_name}: {e}"
            )
            return default
    ...
    def get_environment_int(self, var_name: str, default: int = 0) -> int:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Error getting integer environment variable {var_name}: {e}"
            )
            return default
    ...
    def get_environment_float(self, var_name: str, default: float = 0.0) -> float:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Error getting float environment variable {var_name}: {e}"
            )
            return default
    ...
    def get_storage_directories(self) -> Dict[str, str]:
        ...
        try:
            directories = self.unified_config.get_config_section(
                "storage_settings", "storage_configuration.directories", {}
            )
            if directories:
                return directories
        except Exception as e:
            logger.debug(
                f"Configuration section not found, using environment fallback: {e}"
            )

        # Final fallback to environment variables with enhanced defaults
        return {
            "data_directory": self.unified_config.get_env(
                "NLP_STORAGE_DATA_DIRECTORY", "./data"
            ),
            "cache_directory": self.unified_config.get_env(
                "NLP_STORAGE_CACHE_DIRECTORY", "./cache"
            ),
            "logs_directory": self.unified_config.get_env(
                "NLP_STORAGE_LOG_DIRECTORY", "./logs"
            ),
            "backup_directory": self.unified_config.get_env(
                "NLP_STORAGE_BACKUP_DIRECTORY", "./backups"
            ),
            "models_directory": self.unified_config.get_env(
                "NLP_STORAGE_MODELS_DIR", "./cache/models"
            ),
            "learning_directory": self.unified_config.get_env(
                "NLP_STORAGE_LEARNING_DIRECTORY", "./data/learning"
            ),
        }
    ...
    def get_cache_settings(self) -> Dict[str, Any]:
        ...
        # Enhanced fallback using get_config_section pattern
        try:
            cache_config = self.unified_config.get_config_section(
                "storage_settings", "storage_configuration.cache_settings", {}
            )
            if cache_config:
                return cache_config
        except Exception as e:
            logger.debug(
                f"Cache configuration section not found, using environment fallback: {e}"
            )

        # Fallback to individual environment variables
        return {
            "enable_model_cache": self.unified_config.get_env_bool(
                "NLP_STORAGE_ENABLE_MODEL_CACHE", True
            ),
            "enable_analysis_cache": self.unified_config.get_env_bool(
                "NLP_STORAGE_ENABLE_ANALYSIS_CACHE", False
            ),
            "cache_cleanup_on_startup": self.unified_config.get_env_bool(
                "NLP_STORAGE_CACHE_CLEANUP_ON_STARTUP", False
            ),
            "model_cache_size_limit": self.unified_config.get_env_int(
                "NLP_STORAGE_MODEL_CACHE_SIZE_LIMIT", 1000
            ),
            "analysis_cache_size_limit": self.unified_config.get_env_int(
                "NLP_STORAGE_ANALYSIS_CACHE_SIZE_LIMIT", 500
            ),
            "cache_expiry_hours": self.unified_config.get_env_int(
    ...
    def is_storage_cache_enabled(self) -> bool:
        ...
        try:
            cache_enabled = self.unified_config.get_config_section(
                "storage_settings",
                "storage_configuration.cache_settings.enable_model_cache",
                None,
            )
            ...
            logger.debug(
                f"Cache setting not found in configuration, using environment fallback: {e}"
            )
    ...
    def get_backup_settings(self) -> Dict[str, Any]:
        ...
        # Enhanced fallback using get_config_section pattern
        try:
            backup_config = self.unified_config.get_config_section(
                "storage_settings", "storage_configuration.backup_settings", {}
            )
            if backup_config:
                return backup_config
        except Exception as e:
            logger.debug(
                f"Backup configuration section not found, using environment fallback: {e}"
            )

        # Fallback to individual environment variables
        return {
            "enable_automatic_backup": self.unified_config.get_env_bool(
                "NLP_STORAGE_ENABLE_AUTO_BACKUP", False
            ),
            "backup_interval_hours": self.unified_config.get_env_int(
                "NLP_STORAGE_BACKUP_INTERVAL_HOURS", 24
            ),
            "backup_retention_days": self.unified_config.get_env_int(
                "NLP_STORAGE_BACKUP_RETENTION_DAYS", 30
            ),
            "compress_backups": self.unified_config.get_env_bool(
                "NLP_STORAGE_COMPRESS_BACKUPS", True
            ),
            "backup_learning_data": self.unified_config.get_env_bool(
                "NLP_STORAGE_BACKUP_LEARNING_DATA", True
            ),
            "backup_configuration": self.unified_config.get_env_bool(
    ...
    def get_configuration_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_managers_available": len(
                [
                    m
                    for m in [
                        self.analysis_config_manager,
                        self.pattern_detection_manager,
                        self.feature_config_manager,
                        self.learning_system_manager,
                        self.logging_config_manager,
                        self.model_coordination_manager,
                        self.performance_config_manager,
                        self.pydantic_manager,
                        self.server_config_manager,
                        self.shared_utilities_manager,
                        self.storage_config_manager,
                        self.crisis_threshold_manager,
...
logger.info(
    "✅ SettingsManager v3.1e-5.5-1 loaded - Phase 3e Sub-step 5.5 cleanup complete with enhanced patterns"
)
