...
class SharedUtilitiesManager:
    ...
    def __init__(self, unified_config: UnifiedConfigManager):
        ...
        self.logger.info(
            f"✅ SharedUtilitiesManager initialized (ID: {self.initialization_time})"
        )
    ...
    def safe_bool_convert(
        self, value: Any, default: bool = False, param_name: str = "unknown"
    ) -> bool:
        """
        ...
        try:
            ...
            if isinstance(value, str):
                ...
                if value in true_values:
                    ...
                    self.logger.warning(
                        f"⚠️ Invalid boolean value '{value}' for {param_name}, using default: {default}"
                    )
                    return default
            ...
            self.logger.warning(
                f"⚠️ Cannot convert {type(value).__name__} to boolean for {param_name}, using default: {default}"
            )
            return default

        except Exception as e:
            self.logger.error(
                f"❌ Error converting boolean for {param_name}: {e}, using default: {default}"
            )
            return default

    def get_setting_with_type_conversion(
        self,
        section: str,
        key: str,
        expected_type: type,
        ...
        try:
            ...
            if expected_type == bool:
                ...
                return self.safe_float_convert(
                    raw_value, default, param_name=param_name
                )
            elif expected_type == str:
                return str(raw_value)
            else:
                self.logger.warning(
                    f"⚠️ Unsupported type {expected_type} for {param_name}, returning raw value"
                )
                return raw_value

        except Exception as e:
            self.logger.error(
                f"❌ Error getting setting {param_name}: {e}, using default: {default}"
            )
            return default

    def get_nested_config_setting(
        self, path: str, default: Any = None, fallback_paths: List[str] = None
    ) -> Any:
        """
        ...
        try:
            ...
            if fallback_paths:
                for fallback_path in fallback_paths:
                    ...
                    if value is not None:
                        self.logger.info(
                            f"✅ Used fallback path '{fallback_path}' for '{path}'"
                        )
                        return value

            self.logger.warning(
                f"⚠️ No value found for path '{path}', using default: {default}"
            )
            return default

        except Exception as e:
            self.logger.error(
                f"❌ Error getting nested setting '{path}': {e}, using default: {default}"
            )
            return default

    def get_boolean_setting(
        self, key: str, default: bool = False, environment_override: bool = True
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(
                f"❌ Error getting boolean setting '{key}': {e}, using default: {default}"
            )
            return default
    ...
    def load_json_with_env_substitution(
        self, file_path: str, env_prefix: str = None, required: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if not os.path.exists(file_path):
                if required:
                    ...
                    self.logger.warning(
                        f"⚠️ Optional configuration file not found: {file_path}"
                    )
                    return {}
    ...
    def get_config_section_safely(
        self, section_name: str, fallback: Dict = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if section_data is None:
                if fallback is None:
                    fallback = {}
                self.logger.warning(
                    f"⚠️ Configuration section '{section_name}' not found, using fallback"
                )
                return fallback.copy()

            if not isinstance(section_data, dict):
                self.logger.warning(
                    f"⚠️ Configuration section '{section_name}' is not a dictionary, using fallback"
                )
    ...
    def get_with_fallback(
        self, primary_key: str, fallback_keys: List[str], default: Any = None
    ) -> Any:
        """
        ...
        try:
            ...
            for fallback_key in fallback_keys:
                if fallback_key in config_data:
                    self.logger.info(
                        f"✅ Used fallback key '{fallback_key}' for '{primary_key}'"
                    )
                    return config_data[fallback_key]

            self.logger.warning(
                f"⚠️ No value found for '{primary_key}' or fallbacks, using default: {default}"
            )
            return default

        except Exception as e:
            self.logger.error(
                f"❌ Error getting value with fallback for '{primary_key}': {e}"
            )
            return default

    def validate_config_structure(
        self, config_data: Dict, required_sections: List[str], schema: Dict = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            if issues:
                self.logger.warning(
                    f"⚠️ Configuration validation found {len(issues)} issues"
                )
            else:
    ...
    def _get_safe_fallback_for_operation(self, operation_name: str):
        ...
        if "threshold" in operation_name.lower():
            ...
            "score" in operation_name.lower() or "confidence" in operation_name.lower()
        ):
            return 0.0
    ...
    def safe_int_convert(
        ...
        try:
            if isinstance(value, int):
                ...
                self.logger.warning(
                    f"⚠️ Cannot convert {type(value).__name__} to int for {param_name}"
                )
                return default
            ...
            if min_val is not None and result < min_val:
                self.logger.warning(
                    f"⚠️ Value {result} below minimum {min_val} for {param_name}, using minimum"
                )
                return min_val

            if max_val is not None and result > max_val:
                self.logger.warning(
                    f"⚠️ Value {result} above maximum {max_val} for {param_name}, using maximum"
                )
                return max_val
            ...
            self.logger.error(
                f"❌ Error converting to int for {param_name}: {e}, using default: {default}"
            )
            return default
    ...
    def safe_float_convert(
        ...
        try:
            if isinstance(value, (int, float)):
                ...
                self.logger.warning(
                    f"⚠️ Cannot convert {type(value).__name__} to float for {param_name}"
                )
                return default
            ...
            self.logger.error(
                f"❌ Error converting to float for {param_name}: {e}, using default: {default}"
            )
            return default
    ...
    def validate_range(
        ...
        try:
            if not isinstance(value, (int, float)):
                self.logger.error(
                    f"❌ Non-numeric value for range validation of {param_name}: {type(value)}"
                )
                return False

            if min_val is not None and value < min_val:
                self.logger.warning(
                    f"⚠️ Value {value} below minimum {min_val} for {param_name}"
                )
                return False

            if max_val is not None and value > max_val:
                self.logger.warning(
                    f"⚠️ Value {value} above maximum {max_val} for {param_name}"
                )
                return False
    ...
    def validate_type(
        self, value: Any, expected_type: Union[type, tuple], param_name: str = "unknown"
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.warning(
                f"⚠️ Type mismatch for {param_name}: expected {expected_name}, got {actual_name}"
            )
            return False
    ...
    def validate_bounds(
        ...
        try:
            if not isinstance(value, (int, float)):
                self.logger.error(
                    f"❌ Non-numeric value for bounds validation of {param_name}: {type(value)}"
                )
                return False

            if not isinstance(bounds, dict):
                self.logger.error(
                    f"❌ Invalid bounds dictionary for {param_name}: {type(bounds)}"
                )
                return False
    ...
    def handle_error_with_fallback(
        ...
        try:
            ...
            self.logger.info(
                f"✅ Using fallback value for {operation}: {fallback_value}"
            )
    ...
    def _validate_section_schema(
        self, section_data: Dict, schema: Dict, section_name: str
    ) -> List[str]:
        ...
        try:
            for key, expected_type in schema.items():
                if key not in section_data:
                    ...
                    issues.append(
                        f"Type mismatch for '{section_name}.{key}': expected {expected_name}, got {actual_name}"
                    )
...
def create_shared_utilities_manager(
    ...
    try:
        ...
        logger.info(
            "✅ SharedUtilitiesManager created successfully via factory function"
        )
        return manager
