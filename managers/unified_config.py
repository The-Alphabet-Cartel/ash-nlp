...
class UnifiedConfigManager:
    ...
    def __init__(self, config_dir: str = "/app/config"):
        ...
        self.config_files = {
            ...
            "patterns_config": "patterns_config.json",
        ...
        caching_enabled = (
            os.getenv("NLP_PERFORMANCE_ENABLE_CONFIG_CACHING", "true").lower() == "true"
        )
        if caching_enabled:
            self.caching_helper = create_caching_helper(
                self.config_dir, self.config_files
            )
            self._caching_enabled = True
            logger.info(
                "🚀 UnifiedConfigManager intelligent caching enabled - system-wide performance enhancement"
            )
        else:
        ...
        logger.info(
            "UnifiedConfigManager v3.1e optimized initialized - Helper file architecture with enhanced performance"
        )
    ...
    def _load_all_environment_variables(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Successfully loaded and validated {len(env_config)} environment variables"
        )
        return env_config
    ...
    def _validate_and_convert(self, var_name: str, value: str) -> Any:
        ...
        try:
            ...
            if schema.choices and converted not in schema.choices:
                logger.error(
                    f"Invalid choice for {var_name}: {converted} not in {schema.choices}"
                )
            ...
            if schema.min_value is not None and isinstance(converted, (int, float)):
                if converted < schema.min_value:
                    logger.error(
                        f"Value too low for {var_name}: {converted} < {schema.min_value}"
                    )
                    return schema.default
            ...
            if schema.max_value is not None and isinstance(converted, (int, float)):
                if converted > schema.max_value:
                    logger.error(
                        f"Value too high for {var_name}: {converted} > {schema.max_value}"
                    )
                    return schema.default
    ...
    def get_env(self, var_name: str, default: Any = None) -> Any:
        ...
        if var_name in self.variable_schemas:
            ...
            logger.warning(
                f"No schema found for {var_name}, returning raw value: {env_value}"
            )
            return env_value
    ...
    def get_env_int(self, var_name: str, default: int = 0) -> int:
        ...
        try:
            ...
            logger.warning(
                f"Cannot convert {var_name}={result} to int, using default: {default}"
            )
            return default
    ...
    def get_env_float(self, var_name: str, default: float = 0.0) -> float:
        ...
        try:
            ...
            logger.warning(
                f"Cannot convert {var_name}={result} to float, using default: {default}"
            )
            return default
    ...
    def load_config_file(self, config_name: str) -> Dict[str, Any]:
        ...
        if self._caching_enabled and self.caching_helper:
            ...
            return self.caching_helper.get_cached_config_file(
                config_name, load_function
            )
        else:
    ...
    def _load_config_file_original(self, config_name: str) -> Dict[str, Any]:
        ...
        if not config_file:
            ...
            logger.debug(
                f"Config '{config_name}' not in predefined mapping, trying filename: {config_file}"
            )
        ...
        try:
            ...
            processed_config = self.value_helper.substitute_environment_variables(
                raw_config
            )

            # Apply legacy fallback for any remaining placeholders
            processed_config = self.value_helper.apply_defaults_fallback(
                processed_config
            )
    ...
    def get_config_section(
        self, config_file: str, section_path: str = None, default: Any = None
    ) -> Any:
        """
        ...
        if self._caching_enabled and self.caching_helper:
            ...
            def load_function() -> Any:
                return self._get_config_section_original(
                    config_file, section_path, default
                )

            return self.caching_helper.get_cached_config_section(
                config_file, section_path, default, load_function
            )
        else:
            # Caching disabled - use original method
            return self._get_config_section_original(config_file, section_path, default)

    def _get_config_section_original(
        self, config_file: str, section_path: str = None, default: Any = None
    ) -> Any:
        """
        ...
        try:
            ...
            for part in path_parts:
                if isinstance(result, dict) and part in result:
                    ...
                    logger.debug(
                        f"Section path '{section_path}' not found in '{config_file}', using default"
                    )
                    return default if default is not None else {}
            ...
            if isinstance(result, dict):
                ...
                validated_result = self._apply_json_validation(
                    result, config_file, section_path
                )
                logger.debug(
                    f"Retrieved and validated section '{section_path}' from '{config_file}'"
                )
                return validated_result
            else:
                # Leaf value - find parent section and apply specific validation
                validated_result = self._validate_leaf_value(
                    result, config_file, section_path
                )
                logger.debug(
                    f"Retrieved and validated leaf value '{section_path}' from '{config_file}'"
                )
                return validated_result

        except Exception as e:
            logger.error(
                f"Error getting config section '{section_path}' from '{config_file}': {e}"
            )
            return default if default is not None else {}

    def _validate_leaf_value(
        self, value: Any, config_file: str, section_path: str
    ) -> Any:
        """
        ...
        try:
            validated_value = self._validate_json_setting(
                leaf_key, value, validation_rules[leaf_key]
            )
            logger.debug(
                f"Leaf validation: {config_file}:{section_path} = {value} -> {validated_value} ({type(validated_value).__name__})"
            )
            return validated_value
        except Exception as e:
            logger.warning(
                f"Leaf validation failed for {config_file}:{section_path}: {e}, returning original value"
            )
            return value

    def _apply_json_validation(
        self, data: Any, config_file: str, section_path: str
    ) -> Any:
        """
        Apply JSON validation rules to configuration data
        """
        logger.debug(
            f"_apply_json_validation called: {config_file}, {section_path}, data type: {type(data).__name__}"
        )
        ...
        for key, value in data.items():
            logger.debug(
                f"Processing key '{key}' with value '{value}' (type: {type(value).__name__})"
            )
            ...
            if key in validation_rules:
                logger.debug(
                    f"  Found validation rules for key '{key}': {validation_rules[key]}"
                )
                try:
                    validated_value = self._validate_json_setting(
                        key, value, validation_rules[key]
                    )
                    validated_data[key] = validated_value
                    logger.info(
                        f"Validated {config_file}:{section_path}.{key}: {value} -> {validated_value} ({type(validated_value).__name__})"
                    )
                except Exception as e:
                    logger.warning(
                        f"Validation failed for {config_file}:{section_path}.{key}: {e}, keeping original value"
                    )
                    validated_data[key] = value
            else:
    ...
    def _get_validation_rules(
        self, config_file: str, section_path: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            config_path = self.config_dir / self.config_files.get(
            ...
            if section_path != "root":
                ...
                for part in path_parts:
                    if (
                        isinstance(validation_section, dict)
                        and part in validation_section
                    ):
            ...
            if (
                isinstance(validation_section, dict)
                and "validation" in validation_section
            ):
            ...
            logger.debug(
                f"Error getting validation rules for {config_file}:{section_path}: {e}"
            )
            return {}

    def _validate_json_setting(
        self, setting_name: str, value: Any, validation_rules: Dict[str, Any]
    ) -> Any:
        """
        ...
        if value is None or (
            ...
            if "null" in normalized_types:
                logger.debug(
                    f"JSON validation applied to {setting_name}: '{value}' -> None (allowed null)"
                )
                return None
            else:
                raise ValueError(
                    f"Null value not allowed for {setting_name}: expected types {type_list}"
                )
        ...
        for normalized_type in normalized_types:
            ...
            raise ValueError(
                f"Type conversion failed for {setting_name}: cannot convert '{value}' to any of {type_list}. Errors: {'; '.join(conversion_errors)}"
            )

        # Validate choices
        if choices and converted_value not in choices:
            raise ValueError(
                f"Invalid choice for {setting_name}: '{converted_value}' not in {choices}"
            )
        ...
        if min_value is not None and isinstance(converted_value, (int, float)):
            if converted_value < min_value:
                raise ValueError(
                    f"Value too low for {setting_name}: {converted_value} < {min_value}"
                )

        if max_value is not None and isinstance(converted_value, (int, float)):
            if converted_value > max_value:
                raise ValueError(
                    f"Value too high for {setting_name}: {converted_value} > {max_value}"
                )

        logger.debug(
            f"JSON validation applied to {setting_name}: '{value}' ({type(value).__name__}) -> {converted_value} ({type(converted_value).__name__})"
        )
        return converted_value

    def get_config_section_with_env_fallback(
        self,
        config_file: str,
        section_path: str,
        env_prefix: str = None,
            ...
            if env_prefix:
                ...
                for env_var, env_value in os.environ.items():
                    if env_var.startswith(env_prefix):
                        ...
                        converted_value = self.value_helper.convert_value_type(
                            env_var, env_value
                        )
    ...
    def get_hardware_configuration(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "device": self.get_config_section(
                    "model_coordination", "hardware_settings.device", "auto"
                ),
                "precision": self.get_config_section(
                    "model_coordination", "hardware_settings.model_precision", "float16"
                ),
                "cache_directory": self.get_config_section(
                    "model_coordination",
                    "hardware_settings.cache_dir",
                    "./cache/models",
                ),
            }
        except Exception as e:
    ...
    def get_model_configuration(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "depression_model": self.get_config_section(
                    "model_coordination",
                    "ensemble_models.model_definitions.depression.name",
                    "MoritzLaurer/deberta-v3-base-zeroshot-v2.0",
                ),
                "depression_weight": self.get_config_section(
                    "model_coordination",
                    "ensemble_models.model_definitions.depression.weight",
                    0.4,
                ),
                "sentiment_model": self.get_config_section(
                    "model_coordination",
                    "ensemble_models.model_definitions.sentiment.name",
                    "Lowerated/lm6-deberta-v3-topic-sentiment",
                ),
                "sentiment_weight": self.get_config_section(
                    "model_coordination",
                    "ensemble_models.model_definitions.sentiment.weight",
                    0.3,
                ),
                "emotional_distress_model": self.get_config_section(
                    "model_coordination",
                    "ensemble_models.model_definitions.emotional_distress.name",
                    "MoritzLaurer/mDeBERTa-v3-base-mnli-xnli",
                ),
                "emotional_distress_weight": self.get_config_section(
                    "model_coordination",
                    "ensemble_models.model_definitions.emotional_distress.weight",
                    0.3,
                ),
                "ensemble_mode": self.get_config_section(
                    "model_coordination", "ensemble_config.mode", "majority"
                ),
                "cache_directory": self.get_config_section(
                    "model_coordination",
                    "hardware_settings.cache_dir",
    ...
    def get_performance_configuration(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "request_timeout": self.get_config_section(
                    "performance_settings",
                    "performance_settings.server_performance.request_timeout",
                    30,
                ),
                "max_concurrent_requests": self.get_config_section(
                    "performance_settings",
                    "performance_settings.server_performance.max_concurrent_requests",
                    20,
                ),
                "worker_timeout": self.get_config_section(
                    "performance_settings",
                    "performance_settings.server_performance.worker_timeout",
                    60,
                ),
            }
        except Exception as e:
    ...
    def get_storage_configuration(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "data_directory": self.get_config_section(
                    "storage_settings",
                    "storage_configuration.directories.data_directory",
                    "./data",
                ),
                "cache_directory": self.get_config_section(
                    "storage_settings",
                    "storage_configuration.directories.cache_directory",
                    "./cache",
                ),
                "log_directory": self.get_config_section(
                    "storage_settings",
                    "storage_configuration.directories.logs_directory",
                    "./logs",
                ),
                "backup_directory": self.get_config_section(
                    "storage_settings",
                    "storage_configuration.directories.backup_directory",
                    "./backups",
                ),
                "models_directory": self.get_config_section(
                    "storage_settings",
                    "storage_configuration.directories.models_directory",
                    "./cache/models",
                ),
                "log_file": self.get_config_section(
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "variables_managed": len(
                [
                    k
                    for k in os.environ.keys()
                    if k.startswith("NLP_") or k.startswith("GLOBAL_")
                ]
            ),
            ...
            "optimization_status": {
                ...
                "caching_helper": "managers/helpers/unified_config_caching_helper.py"
            ...
            "schema_system": {
                ...
                "json_driven_schemas": len(self.variable_schemas)
                - self.schema_helper.count_core_schemas(),
                "helper_managed": True,
        ...
        if self._caching_enabled and self.caching_helper:
            try:
                ...
                status["caching"] = {
                    ...
                    "performance_improvement_factor": cache_stats[
                ...
                if cache_stats["total_requests"] > 0:
                    status["caching"]["performance_summary"] = (
                        f"Cache improving config access by {cache_stats['performance_improvement']:.1f}x "
            ...
            status["caching"] = {
                "enabled": False,
                "reason": "disabled by NLP_PERFORMANCE_ENABLE_CONFIG_CACHING=false",
            }
