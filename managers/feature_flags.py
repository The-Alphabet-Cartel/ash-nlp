...
class FeatureConfigManager:
    ...
    def __init__(self, config_manager):
        ...
        logger.info(
            "Initializing FeatureConfigManager v3.1e-5.5 with Phase 3e patterns"
        )
    ...
    def _load_feature_configuration(self):
        ...
        try:
            ...
            logger.debug(
                "Feature flags configuration loaded successfully using Phase 3e patterns"
            )
            logger.debug(
                f"Configuration sections loaded: {list(self.config_cache.keys())}"
            )
    ...
    def _initialize_safe_defaults(self):
        ...
        self.config_cache = {
            ...
            "learning_features": {
                ...
                "defaults": {"threshold_learning": False, "pattern_learning": False},
            },
            "experimental_features": {
    ...
    def _validate_feature_dependencies(self):
        ...
        try:
            ...
            dependencies_config = self.config_manager.get_config_section(
                "feature_flags", "feature_dependencies", {}
            )
            dependencies = self.config_manager.get_config_section(
                "feature_flags", "feature_dependencies.dependencies", {}
            )
            conflicts = self.config_manager.get_config_section(
                "feature_flags", "feature_dependencies.conflicts", {}
            )
                ...
                if self.is_feature_enabled(feature):
                    for conflicting in conflict_list:
                        if self.is_feature_enabled(conflicting):
                            error_msg = (
                                f"Feature '{feature}' conflicts with '{conflicting}'"
                            )
                            self.validation_errors.append(error_msg)
            ...
            if self.validation_errors:
                logger.warning(
                    f"Feature validation found {len(self.validation_errors)} issues"
                )
            else:
    ...
    def is_pattern_integration_enabled(self) -> bool:
        ...
        return self._get_feature_flag(
            "core_system_features", "pattern_integration", True
        )

    def is_staff_review_logic_enabled(self) -> bool:
        """Check if staff review logic is enabled"""
        return self._get_feature_flag(
            "core_system_features", "staff_review_logic", True
        )
    ...
    def get_learning_features(self) -> Dict[str, bool]:
        ...
        try:
            ...
            return {"threshold_learning": False, "pattern_learning": False}
    ...
    def is_advanced_context_enabled(self) -> bool:
        ...
        return self._get_feature_flag(
            "experimental_features", "advanced_context", False
        )
    ...
    def is_temporal_patterns_enabled(self) -> bool:
        ...
        return self._get_feature_flag(
            "experimental_features", "temporal_patterns", True
        )
    ...
    def get_feature_categories(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            categories = self.config_manager.get_config_section(
                "feature_flags", "feature_categories", {}
            )
            if not categories:
    ...
    def get_configuration_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "total_features": sum(
                    len(category) for category in all_features.values()
                ),
    ...
    def _get_feature_flag(self, category: str, feature: str, default: bool) -> bool:
        ...
        """
        try:
            category_config = self.config_manager.get_config_section(
                "feature_flags", f"features.{category}", {}
            )

            # First try to get the value directly (after environment substitution)
            value = self.config_manager.get_config_section(
                "feature_flags", f"features.{category}.{feature}", {}
            )
            ...
            logger.warning(
                f"Error getting feature flag {category}.{feature}: {e}, using default: {default}"
            )
            return default
...
logger.info(
    "FeatureConfigManager v3.1e-5.5-3 loaded - Phase 3e Sub-step 5.5 cleanup complete with enhanced patterns"
)
