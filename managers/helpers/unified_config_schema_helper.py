# ash-nlp/managers/helpers/unified_config_schema_helper.py
"""
...
class UnifiedConfigSchemaHelper:
    ...
    def initialize_schemas(self) -> Dict[str, VariableSchema]:
        ...
        logger.info(
            f"Initialized {len(schemas)} schemas ({core_count} core + {json_count} JSON-driven)"
        )
        return schemas
    ...
    def get_essential_core_schemas(self) -> Dict[str, VariableSchema]:
        ...
        return {
            ...
            "GLOBAL_LOG_LEVEL": VariableSchema(
                "str",
                "INFO",
                choices=["DEBUG", "INFO", "WARNING", "ERROR"],
                description="Global logging level for Ash ecosystem",
            ),
            "GLOBAL_DEBUG": VariableSchema(
                "bool", False, description="Global debug mode for Ash ecosystem"
            ),
            "GLOBAL_ENABLE_LOGGING": VariableSchema(
                "bool", True, description="Global logging enablement for Ash ecosystem"
            ),
            "GLOBAL_NLP_API_PORT": VariableSchema(
                "int",
                8881,
                min_value=1024,
                max_value=65535,
            ...
            "GLOBAL_ENABLE_CORS": VariableSchema(
                "bool", True, description="Global CORS enablement for Ash ecosystem"
            ),
            "GLOBAL_ALLOWED_IPS": VariableSchema(
                "str",
                "10.20.30.0/24,127.0.0.1,::1",
                description="Global allowed IP ranges for Ash ecosystem",
            ),
            "GLOBAL_HUGGINGFACE_TOKEN": VariableSchema(
                "str", "", description="Global HuggingFace token for Ash ecosystem"
            ),
            "GLOBAL_FEATURE_ENABLE_LEARNING_SYSTEM": VariableSchema(
                "bool",
                True,
                description="Global learning system enablement for Ash ecosystem",
            ),
            # Core Server Variables (Needed for system startup)
            "NLP_SERVER_HOST": VariableSchema(
                "str", "0.0.0.0", description="Server bind address for NLP service"
            ),
            "NLP_SERVER_PORT": VariableSchema(
                "int",
                8881,
                min_value=1024,
                max_value=65535,
            ...
            "NLP_SERVER_WORKERS": VariableSchema(
                "int",
                1,
                min_value=1,
                max_value=16,
            ...
            # Context pattern variables that are actively used in JSON configs
            # These must be in essential core schemas because they're referenced in placeholder resolution
            "NLP_CONFIG_ENHANCED_CRISIS_WEIGHT": VariableSchema(
                "float",
                1.2,
                min_value=0.1,
                max_value=5.0,
            ...
            "NLP_HOPELESSNESS_CONTEXT_CRISIS_BOOST": VariableSchema(
                "float",
                1.2,
                min_value=0.1,
                max_value=5.0,
            ...
            "NLP_HOPELESSNESS_CONTEXT_BOOST_FACTOR": VariableSchema(
                "float",
                1.2,
                min_value=0.1,
                max_value=5.0,
        ...
        for config_name, config_file in self.config_files.items():
            try:
                ...
                if file_schemas:
                    ...
                    logger.debug(
                        f"Loaded {len(file_schemas)} schemas from {config_name}"
                    )
    ...
    def extract_validation_schemas(
        ...
        for key, value in config.items():
            ...
            if key == "validation" and isinstance(value, dict):
                ...
                validation_blocks.extend(
                    self.find_validation_blocks(value, current_path)
                )

        return validation_blocks

    def json_to_schema(
        self, json_rules: Dict[str, Any], var_name: str
    ) -> VariableSchema:
        """
