# ash-nlp/managers/helpers/unified_config_value_helper.py
"""
...
class UnifiedConfigValueHelper:
        ...
        if value.replace(".", "").replace("-", "").isdigit():
            try:
                if "." in value:
                    ...
                    logger.debug(
                        f"   {env_var} -> Float conversion: {value} -> {result}"
                    )
                    return result
                else:
    ...
    def substitute_environment_variables(
        self, value: Any, defaults_context: Dict[str, Any] = None
    ) -> Any:
        """
        ...
        if isinstance(value, str):
            ...
            return [
                self.substitute_environment_variables(item, defaults_context)
                for item in value
            ]
    ...
    def apply_defaults_fallback(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def apply_legacy_defaults_recursive(
            main_value: Any, defaults_value: Any
        ) -> Any:
            if (
                isinstance(main_value, str)
                and main_value.startswith("${")
                and main_value.endswith("}")
            ):
                ...
                for key in main_value:
                    if key in defaults_value:
                        result[key] = apply_legacy_defaults_recursive(
                            main_value[key], defaults_value[key]
                        )
                    else:
