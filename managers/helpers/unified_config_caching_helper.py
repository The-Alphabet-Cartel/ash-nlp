# ash-nlp/managers/helpers/unified_config_caching_helper.py
"""
...
class UnifiedConfigCachingHelper:
    ...
    def __init__(self, config_dir: Path, config_files: Dict[str, str]):
        ...
        self.max_entries = int(os.getenv("NLP_CONFIG_CACHE_MAX_ENTRIES", "50"))
        self.cache_ttl = int(
            os.getenv("NLP_CONFIG_CACHE_TTL_SECONDS", "300")
        )  # 5 minutes
        self.max_memory_mb = int(os.getenv("NLP_CONFIG_CACHE_MAX_MEMORY_MB", "25"))
        self.max_memory_bytes = self.max_memory_mb * 1024 * 1024
        self.enable_file_watching = (
            os.getenv("NLP_CONFIG_CACHE_FILE_WATCHING", "true").lower() == "true"
        )
        ...
        logger.info(
            f"UnifiedConfigCachingHelper initialized: max_entries={self.max_entries}, "
    ...
    # GET_ METHODS
    # ============================================================================
    def get_cached_config_file(
        self, config_name: str, load_function: Callable[[], Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        with self._cache_lock:
            ...
            if cache_key in self._config_cache:
                ...
                if self._is_cache_entry_valid(entry, config_name):
                    ...
                    logger.debug(
                        f"Cache hit: {config_name} (access #{entry.access_count}, {access_time:.3f}ms)"
                    )
                    return entry.data
                else:
            ...
            if config_data:
                ...
                logger.debug(
                    f"Cached config file: {config_name} ({size_estimate} bytes, {load_time:.2f}ms load time)"
                )

            logger.debug(f"Cache miss: {config_name} ({load_time:.2f}ms)")
            return config_data

    def get_cached_config_section(
        self,
        config_file: str,
        section_path: str,
        default: Any,
        ...
        with self._cache_lock:
            ...
            if cache_key in self._config_cache:
                ...
                if self._is_cache_entry_valid(entry, config_file):
                    ...
                    logger.debug(
                        f"Cache hit: {cache_key} (access #{entry.access_count}, {access_time:.3f}ms)"
                    )
                    return entry.data
                else:
                    # Cache entry invalid - remove it
            ...
            if section_data != default:
                ...
                self._config_cache[cache_key] = entry
                logger.debug(
                    f"Cached config section: {cache_key} ({size_estimate} bytes, {load_time:.2f}ms load time)"
                )
    ...
    def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        with self._cache_lock:
            current_size = sum(
                entry.size_estimate for entry in self._config_cache.values()
            )
            ...
            return {
                ...
                "performance_improvement": round(
                    self._stats.performance_improvement, 1
                ),
                ...
                "memory_utilization_pct": round(
                    (current_size / self.max_memory_bytes) * 100, 1
                ),
    ...
    def _estimate_object_size(self, obj: Any) -> int:
        ...
        try:
            if isinstance(obj, dict):
                return (
                    sum(
                        len(str(k)) + self._estimate_object_size(v)
    ...
    def _is_cache_entry_valid(self, entry: CacheEntry, config_name: str) -> bool:
        ...
        if self.enable_file_watching:
            ...
            if current_mtime > (entry.file_mtime + mtime_epsilon):
                logger.debug(
                    f"Cache entry invalidated (file modified): {config_name}, "
            ...
            if current_mtime == 0.0 and entry.file_mtime > 0.0:
                logger.debug(
                    f"Cache entry invalidated (file no longer accessible): {config_name}"
                )
                return False
    ...
    def _evict_lru_entries(self, space_needed: int = 0) -> None:
        ...
        while (
            current_size + space_needed > self.max_memory_bytes
    ...
    def clear_cache(self, pattern: str = None) -> int:
        ...
        with self._cache_lock:
            if pattern is None:
                ...
                logger.info(
                    f"Cleared cache entries matching '{pattern}': {len(keys_to_remove)}"
                )
    ...
    def invalidate_config(self, config_name: str) -> int:
        ...
        with self._cache_lock:
            ...
            if keys_to_remove:
                logger.info(
                    f"Invalidated {len(keys_to_remove)} cache entries for config: {config_name}"
                )
...
# FACTORY
# ============================================================================
def create_caching_helper(
    config_dir: Path, config_files: Dict[str, str]
...
logger.info(
    "UnifiedConfigCachingHelper loaded - Intelligent caching for configuration management"
)
# ============================================================================
