...
class CrisisThresholdManager:
    ...
    def __init__(self, unified_config_manager, model_coordination_manager=None):
        ...
        logger.info(
            "CrisisThresholdManager v3.1-3e-5.7-2 initialized - Phase 3e Step 5.7 manager renaming complete"
        )
    ...
    def _load_crisis_threshold_config(self):
        ...
        """
        try:
            self.threshold_config = self.unified_config.get_config_section(
                "crisis_threshold"
            )
            logger.info(
                "✅ Threshold mapping configuration loaded (environment resolution via get_config_section)"
            )
    ...
    def _validate_threshold_config(self):
        ...
        try:
                ...
                try:
                    ...
                    if ensemble_thresholds:
                        ...
                        if not (low <= medium <= high <= critical):
                            ...
                            logger.debug(
                                f"✅ Threshold ordering valid for {mode}: {low} <= {medium} <= {high} <= {critical}"
                            )
                    ...
                    if staff_thresholds:
                        medium_conf = staff_thresholds.get(
                            "medium_confidence_threshold", 0.5
                        )
                        low_conf = staff_thresholds.get("low_confidence_threshold", 0.8)

                        if not (0.0 <= medium_conf <= 1.0) or not (
                            0.0 <= low_conf <= 1.0
                        ):
            ...
            if self._validation_errors:
                logger.warning(
                    f"⚠️ Threshold validation found {len(self._validation_errors)} issues"
                )
            else:
    ...
    def determine_crisis_level(self, score: float, mode: Optional[str] = None) -> str:
        ...
        try:
            ...
            logger.debug(
                f"🎯 Adjusted thresholds for {mode}: critical={adjusted_critical:.3f}, high={adjusted_high:.3f}, medium={adjusted_medium:.3f}, low={adjusted_low:.3f}"
            )
            ...
            logger.info(
                f"🎯 ENHANCED Crisis level determination: score={score:.3f} → {crisis_level} (mode: {mode}, adjusted thresholds)"
            )
            return crisis_level
    ...
    def _fallback_crisis_level_determination(self, score: float) -> str:
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Invalid score value: {score}, returning 'medium' for safety"
            )
        ...
        try:
            ...
            for key, value in ensemble_thresholds.items():
                try:
                    ...
                    logger.warning(
                        f"⚠️ Invalid ensemble threshold '{value}' for {key} in mode '{mode}', using default {default_val}"
                    )
            ...
            return {"critical": 0.7, "high": 0.45, "medium": 0.25, "low": 0.12}

    def get_crisis_level_mapping_for_mode(
        ...
        try:
            ...
            for key, value in crisis_mapping.items():
                try:
                    ...
                    logger.warning(
                        f"⚠️ Invalid {key} value '{value}' for mode '{mode}', using default {default_val}"
                    )
            ...
            logger.error(
                f"❌ Error getting crisis level mapping for mode '{mode}': {e}"
            )
            return {
                "crisis_to_high": 0.5,
                "crisis_to_medium": 0.3,
    ...
    def get_staff_review_thresholds_for_mode(
        self, mode: Optional[str] = None
        ...
        try:
            ...
            if not staff_thresholds:
                logger.debug(
                    f"📋 No mode-specific staff review thresholds for '{mode}', using global defaults"
                )
            ...
            bool_keys = [
                "high_always",
                "on_model_disagreement",
                "gap_detection_review",
                "pattern_mismatch_review",
            ]
                ...
                if key in staff_thresholds:
                    ...
                    try:
                        if isinstance(value, str):
                            safe_thresholds[key] = value.lower() in [
                                "true",
                                "1",
                                "yes",
                                "on",
                        ...
                        logger.warning(
                            f"⚠️ Invalid boolean value '{value}' for {key}, using True"
                        )
            ...
            for key, default_val in float_keys_defaults.items():
                if key in staff_thresholds:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"⚠️ Invalid float value '{value}' for {key}, using default {default_val}"
                        )
                        safe_thresholds[key] = default_val

            # Ensure we have the expected keys with consistent naming
            if "medium_confidence_threshold" not in safe_thresholds:
                safe_thresholds["medium_confidence_threshold"] = safe_thresholds.get(
                    "medium_confidence", 0.5
                )
            if "low_confidence_threshold" not in safe_thresholds:
                safe_thresholds["low_confidence_threshold"] = safe_thresholds.get(
                    "low_confidence", 0.8
                )

            logger.debug(
                f"✅ Staff review thresholds for mode '{mode}': {safe_thresholds}"
            )
            return safe_thresholds

        except Exception as e:
            logger.error(
                f"❌ Error getting staff review thresholds for mode '{mode}': {e}"
            )
            return self.get_global_staff_review_thresholds()
    ...
    def get_staff_review_config(self) -> Dict[str, Union[bool, float]]:
        ...
        try:
            ...
            config = {
                ...
                "medium_confidence_threshold": staff_thresholds.get(
                    "medium_confidence", 0.45
                ),
                "low_confidence_threshold": staff_thresholds.get(
                    "low_confidence", 0.75
                ),
                "on_disagreement": staff_thresholds.get("on_disagreement", True),
                "gap_detection_review": staff_thresholds.get(
                    "gap_detection_review", True
        ...
        try:
            ...
            for key in bool_keys:
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ Invalid boolean value '{value}' for global {key}, using True"
                    )
                    safe_global[key] = True

            # Float values
            float_keys_defaults = {"medium_confidence": 0.75, "low_confidence": 0.5}
            ...
            for key, default_val in float_keys_defaults.items():
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ Invalid float value '{value}' for global {key}, using default {default_val}"
                    )
    ...
    def is_staff_review_required(
        self,
        crisis_level: str,
        confidence: float,
        has_model_disagreement: bool = False,
        has_gap_detection: bool = False,
    ) -> bool:
        """
        ...
        try:
            ...
            if crisis_level == "high" and staff_config.get("high_always", True):
                logger.debug(
                    f"📋 Staff review required: High crisis level (always={staff_config.get('high_always')})"
                )
                return True
            ...
            if crisis_level == "medium":
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ Invalid medium_confidence_threshold '{medium_threshold}', using default 0.45"
                    )
                    medium_threshold = 0.45

                if confidence < medium_threshold:
                    logger.debug(
                        f"📋 Staff review required: Medium crisis with low confidence ({confidence:.3f} < {medium_threshold:.3f})"
                    )
                    return True
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ Invalid low_confidence_threshold '{low_threshold}', using default 0.75"
                    )
                    low_threshold = 0.75

                if confidence < low_threshold:
                    logger.debug(
                        f"📋 Staff review required: Low crisis with very low confidence ({confidence:.3f} < {low_threshold:.3f})"
                    )
                    return True
            ...
            try:
                ...
                if crisis_level == "medium":
                    ...
                    try:
                        ...
                        logger.warning(
                            f"⚠️ Invalid crisis_to_high threshold '{high_threshold_raw}', using default 0.5"
                        )
                        high_threshold = 0.5
                    ...
                    if confidence >= (high_threshold - 0.05):
                        logger.debug(
                            f"📋 Staff review required: Near high threshold boundary ({confidence:.3f} near {high_threshold:.3f})"
                        )
                        return True

            except Exception as threshold_error:
                logger.warning(
                    f"⚠️ Error checking threshold boundaries: {threshold_error}"
                )
            ...
            if crisis_level == "none" and confidence >= 0.9:
                logger.debug(
                    f"📋 Staff review required: Very high confidence with no crisis detected (potential false negative)"
                )
                return True

            logger.debug(
                f"📋 No staff review required: {crisis_level} level, confidence {confidence:.3f}"
            )
            return False
    ...
    def get_pattern_integration_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for key, default_val in float_keys_defaults.items():
                ...
                try:
                    ...
                    if key == "confidence_boost_limit" and safe_config[key] > 1.0:
                        logger.warning(
                            f"⚠️ {key} value {safe_config[key]} exceeds 1.0, capping at 1.0"
                        )
                        safe_config[key] = 1.0
                    elif key in ["pattern_override_threshold"] and not (
                        0.0 <= safe_config[key] <= 1.0
                    ):
                        logger.warning(
                            f"⚠️ {key} value {safe_config[key]} out of range [0,1], using default"
                        )
            ...
            # String values
            safe_config["escalation_required_minimum"] = integration_config.get(
                "escalation_required_minimum", "low"
            )
            if safe_config["escalation_required_minimum"] not in [
                "none",
                "low",
                "medium",
                "high",
                ...
                logger.warning(
                    f"⚠️ Invalid escalation_required_minimum '{safe_config['escalation_required_minimum']}', using 'low'"
                )
            ...
            safe_config.update(
                {
                    "enabled": True,
                    "confidence_boost_multiplier": safe_config[
                        "pattern_weight_multiplier"
                    ],
                    "pattern_weight": 0.3,
                    "ensemble_weight": 0.7,
            ...
            logger.debug(
                f"✅ Pattern integration config loaded: multiplier={safe_config['pattern_weight_multiplier']}"
            )
            return safe_config
            ...
            return {
                ...
                "ensemble_weight": 0.7,
                "confidence_threshold": 0.5,
                "crisis_level_promotion": {"enabled": False, "boost_factor": 0.0},
            }
    ...
    def get_safety_controls_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"✅ Safety controls config loaded: enabled={safety_config.get('enabled', True)}"
            )
    ...
    def get_validation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "modes_configured": list(
                self.threshold_config.get("crisis_threshold_by_mode", {}).keys()
            )
...
def create_crisis_threshold_manager(
    unified_config_manager, model_coordination_manager=None
...
__all__ = ["CrisisThresholdManager", "create_crisis_threshold_manager"]

logger.info(
    "✅ CrisisThresholdManager v3.1-3e-5.7-2 loaded - Phase 3e Step 5.7 manager renaming complete"
)
