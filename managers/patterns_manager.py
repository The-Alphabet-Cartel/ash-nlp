...
from managers.helpers.pattern_detection_helper import (
    CrisisPatternHelper,
    create_pattern_detection_helper,
)
...
class PatternDetectionManager:
    ...
    def __init__(self, config_manager: UnifiedConfigManager):
        ...
        logger.info(
            f"PatternDetectionManager initialized with {len(self._patterns_cache)} pattern sets"
        )
        ...
        for pattern_type in pattern_files:
            try:
                ...
                if patterns:
                    ...
                    logger.debug(
                        f"Loaded {pattern_type}: {len(patterns.get('patterns', {}) if isinstance(patterns.get('patterns'), dict) else patterns)} pattern groups"
                    )
                else:
    ...
    def analyze_enhanced_patterns(self, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            case_sensitive = self._helpers.safe_get_bool(
                processing_rules,
                "case_sensitive",
                defaults.get("case_sensitive", False),
            )
            context_window = self._helpers.safe_get_int(
                processing_rules, "context_window", defaults.get("context_window", 10)
            )
            max_pattern_boost = self._helpers.safe_get_float(
                processing_rules,
                "max_pattern_boost",
                defaults.get("max_pattern_boost", 0.50),
            )
            ...
            logger.debug(
                f"🔍 Enhanced pattern analysis: {len(patterns)} pattern categories"
            )
            ...
            for pattern_group, pattern_data in patterns.items():
                ...
                crisis_level = pattern_data.get(
                    "crisis_level", category_defaults.get("crisis_level", "medium")
                )
                category_description = pattern_data.get(
                    "description", f"{pattern_group} patterns"
                )
                ...
                for pattern_item in pattern_data["patterns"]:
                    ...
                    if pattern_type == "regex":
                        try:
                            ...
                            if cache_key not in self._compiled_regex_cache:
                                ...
                                self._compiled_regex_cache[cache_key] = re.compile(
                                    pattern_text, flags
                                )
                            ...
                            logger.warning(
                                f"⚠️ Invalid regex pattern '{pattern_text}': {e}"
                            )
                            continue
                        ...
                        search_text = (
                            pattern_text if case_sensitive else pattern_text.lower()
                        )
                    ...
                    if matched:
                        ...
                        weight = float(
                            pattern_item.get(
                                "weight", category_defaults.get("weight_medium", 0.7)
                            )
                        )
                        urgency = pattern_item.get(
                            "urgency", category_defaults.get("urgency", "medium")
                        )
                        auto_escalate = pattern_item.get(
                            "auto_escalate",
                            category_defaults.get("auto_escalate", False),
                        )
                        context_required = pattern_item.get(
                            "context_required",
                            category_defaults.get("context_required", False),
                        )
                        ...
                        pattern_result = {
                            ...
                            "confidence": min(
                                weight + 0.1, 1.0
                            ),  # Slight confidence boost for matches
                        ...
                        if crisis_level == "critical":
                            safety_flags["critical_patterns_detected"].append(
                                pattern_text
                            )
                            safety_flags["emergency_response_triggered"] = True

                        if auto_escalate:
                            safety_flags["auto_escalation_required"].append(
                                pattern_text
                            )

                        if urgency == "critical":
                            safety_flags["immediate_intervention_patterns"].append(
                                pattern_text
                            )

                        logger.info(
                            f"🚨 ENHANCED PATTERN MATCH: {pattern_group} → {pattern_text} (level: {crisis_level})"
                        )
            ...
            if patterns_found:
                ...
                confidence_score = min(
                    sum(confidence_values) / len(confidence_values), 1.0
                )

            # Check emergency response threshold with more aggressive defaults
            emergency_threshold = self._helpers.safe_get_float(
                escalation_rules,
                "emergency_response_threshold",
                escalation_rules.get("defaults", {}).get(
                    "emergency_response_threshold", 0.60
                ),
            )

            if (
                confidence_score >= emergency_threshold
                or safety_flags["critical_patterns_detected"]
            ):
                safety_flags["emergency_response_triggered"] = True
            ...
            analysis_result = {
                ...
                "high_confidence_patterns": [
                    p for p in patterns_found if p.get("confidence", 0) > 0.8
                ],
                "critical_patterns": [
                    p for p in patterns_found if p.get("crisis_level") == "critical"
                ],
                "auto_escalation_patterns": [
                    p for p in patterns_found if p.get("auto_escalate", False)
                ],
                "processing_info": {
            ...
            if safety_flags["emergency_response_triggered"]:
                logger.critical(
                    f"🚨 EMERGENCY RESPONSE TRIGGERED - Critical patterns: {safety_flags['critical_patterns_detected']}"
                )
    ...
    def apply_context_weights(
        self, message: str, base_crisis_score: float
        ...
        try:
            ...
            try:
                context_boost_weight = self.config_manager.get_config_section(
                    "analysis_config", "semantic_analysis.context_boost_weight", 1.5
                )
                crisis_base_weight = context_boost_weight * 0.1
                crisis_multiplier = self.config_manager.get_config_section(
                    "patterns_context", "configuration.crisis_amplification", 1.0
                )
                max_boost = crisis_base_weight * crisis_multiplier * 2.0

            except Exception as e:
                logger.warning(
                    f"Error reading existing environment variables: {e}, using safe defaults"
                )
            ...
            if crisis_words and "words" in crisis_words:
                ...
                for word in amplifier_words:
                    if isinstance(word, str) and word.lower() in message_lower:
                        adjustment = min(
                            crisis_base_weight, max_boost - total_adjustment
                        )
                        if adjustment > 0:
            ...
            if positive_words and "words" in positive_words:
                ...
                for word in reducer_words:
                    if isinstance(word, str) and word.lower() in message_lower:
                        adjustment = max(
                            positive_base_weight, max_reduction - total_adjustment
                        )
                        if adjustment < 0:
                            total_adjustment += adjustment
                            weights_applied.append(
            ...
            if any(
                ...
                for word in ["hopeless", "hope", "despair", "desperate"]
            ):
                try:
                    enhanced_weight = self.config_manager.get_config_section(
                        "patterns_contexthopelessness_context.enhanced_weight", 1.2
                    )
            ...
            analysis_details = {
                ...
                "crisis_words_found": len(
                    [w for w in weights_applied if w["type"] == "crisis_amplifier"]
                ),
                "positive_words_found": len(
                    [w for w in weights_applied if w["type"] == "positive_reducer"]
                ),
                "enhanced_boosts": len(
                    [w for w in weights_applied if w["type"] == "enhanced_crisis_boost"]
                ),
            ...
            if total_adjustment != 0:
                logger.info(
                    f"Context weights (using existing env vars): {base_crisis_score:.3f} → {modified_score:.3f} (Δ{total_adjustment:+.3f})"
                )
    ...
    def check_patterns_crisis(self, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if urgency_levels:
                highest_urgency = max(
                    urgency_levels, key=lambda x: urgency_priority.get(x, 0)
                )
            ...
            requires_immediate_attention = (
                safety_flags.get("emergency_response_triggered", False)
                or safety_flags.get("immediate_intervention_patterns", [])
                or highest_urgency == "critical"
                or any(m.get("crisis_level") == "critical" for m in matches)
            )
            ...
            result = {
                ...
                "safety_flags": safety_flags,
                "critical_patterns_count": len(
                    safety_flags.get("critical_patterns_detected", [])
                ),
            ...
            if matches:
                logger.info(
                    f"Enhanced crisis patterns check: {len(matches)} matches, urgency: {highest_urgency}"
                )
    ...
    def get_patterns_context(self) -> Dict[str, Any]:
        ...
        return {"patterns": "Not Found!", "configuration": "Not Found!"}
        ...
        if consolidated:
            ...
            if "crisis_amplifier_words" in crisis_amp:
                ...
                weights["crisis_context_words"] = {
                    ...
                    "max_cumulative_boost": amplifier_data.get(
            ...
            if "positive_reducer_words" in positive_red:
                ...
                weights["positive_context_words"] = {
                    ...
                    "max_cumulative_reduction": reducer_data.get(
                        "max_cumulative_reduction", -0.25
                    ),
                ...
                if isinstance(pattern_data, dict) and "patterns" in pattern_data:
                    ...
                    if isinstance(patterns_section, dict):
                        for group_name, group_patterns in patterns_section.items():
                            if (
                                isinstance(group_patterns, dict)
                                and "patterns" in group_patterns
                            ):
    ...
    def extract_crisis_context_phrases(self, message: str) -> List[Dict[str, Any]]:
        ...
        return self._helpers.extract_crisis_context_phrases(
            message, self._patterns_cache
        )
    ...
    def analyze_message(
        self, message: str, user_id: str = "unknown", channel_id: str = "unknown"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"🔍 Starting comprehensive Phase 3e analysis for message (length: {len(message)})"
            )
            ...
            try:
                ...
                if enhanced_analysis and enhanced_analysis.get("patterns_found"):
                    for pattern in enhanced_analysis.get("patterns_found", []):
                        ...
                        if pattern.get("crisis_level") == "critical":
                            analysis_result["safety_assessment"][
                                "critical_patterns_detected"
                            ].append(pattern_data["pattern_name"])
                ...
                if safety_flags.get("emergency_response_triggered", False):
                    analysis_result["safety_assessment"][
                        "emergency_response_recommended"
                    ] = True
                ...
                if temporal_analysis and temporal_analysis.get("found_indicators"):
                    for indicator in temporal_analysis.get("found_indicators", []):
                        pattern_data = {
                            ...
                            "urgency_multiplier": indicator.get(
                        ...
                        if indicator.get("time_sensitivity") in [
                            "immediate",
                            "critical",
                        ]:
                            analysis_result["safety_assessment"][
            ...
            crisis_levels = [
                p.get("crisis_level", "low")
                for p in analysis_result["patterns_triggered"]
            ]
            ...
            if crisis_levels:
                highest_level = max(
                    crisis_levels, key=lambda x: level_priority.get(x, 0)
                )
            ...
            if (
                ...
                or analysis_result["safety_assessment"][
                    "immediate_intervention_required"
                ]
                or len(
                    analysis_result["safety_assessment"]["critical_patterns_detected"]
                )
                > 0
            ):
                analysis_result["safety_assessment"][
            ...
            analysis_result.update(
                {
                    "summary": {
                        ...
                        "requires_attention": highest_level
                        in ["critical", "high", "medium"]
                        or pattern_count >= 3,
                        "requires_immediate_response": analysis_result[
                            "safety_assessment"
                        ]["immediate_intervention_required"],
                        "requires_emergency_response": analysis_result[
                            "safety_assessment"
                        ]["emergency_response_recommended"],
                        "auto_escalation_needed": analysis_result["safety_assessment"][
                            "auto_escalation_triggered"
                        ],
                        "critical_pattern_count": len(
                            analysis_result["safety_assessment"][
                                "critical_patterns_detected"
                            ]
                        ),
            ...
            if analysis_result["safety_assessment"]["immediate_intervention_required"]:
                logger.critical(
                    f"🚨 IMMEDIATE INTERVENTION REQUIRED - User: {user_id}, Channel: {channel_id}"
                )
            elif analysis_result["safety_assessment"]["emergency_response_recommended"]:
                logger.error(
                    f"⚠️ EMERGENCY RESPONSE RECOMMENDED - Highest level: {highest_level}, User: {user_id}"
                )
            elif highest_level in ["high", "medium"]:
                logger.warning(
                    f"⚠️ Crisis patterns detected - Level: {highest_level}, Count: {pattern_count}, User: {user_id}"
                )

            logger.debug(
                f"✅ Phase 3e crisis analysis complete: {pattern_count} patterns, level: {highest_level}"
            )
            return analysis_result
    ...
    def find_triggered_patterns(
        self, message: str, model_coordination_manager=None
    ) -> List[Dict[str, Any]]:
        """Find triggered crisis patterns using semantic NLP classification"""
        try:
            if model_coordination_manager:
                semantic_patterns = self._helpers.find_patterns_semantic(
                    message, model_coordination_manager
                )
                if semantic_patterns:
                    logger.info(
                        f"✅ Semantic classification found {len(semantic_patterns)} patterns"
                    )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            has_legacy_context_files = any(
                f in self._patterns_cache
                for f in [
                    "patterns_context",
                    "positive_patterns",
            ...
            has_legacy_community_files = any(
                f in self._patterns_cache
                for f in ["crisis_lgbtqia_patterns", "patterns_community"]
            )
            ...
            return {
                ...
                "consolidation_status": {
                    ...
                    "context_consolidation_complete": has_consolidated_context
                    and not has_legacy_context_files,
                    ...
                    "total_files_eliminated": (3 if has_consolidated_context else 0)
                    + 2,
                    "pattern_ecosystem_files": len(
                        [
                            f
                            for f in self._patterns_cache.keys()
                            if not f.startswith("crisis_context")
                            and not f.startswith("positive_context")
                            and not f.startswith("context_weights")
                            and not f == "crisis_lgbtqia_patterns"
                            and not f == "patterns_community"
                        ]
                    ),
                },
...
def create_pattern_detection_manager(
    config_manager: UnifiedConfigManager,
...
logger.info(
    "✅ PatternDetectionManager v3.1-3e-5.3-optimized loaded - Hybrid optimization complete!"
)
